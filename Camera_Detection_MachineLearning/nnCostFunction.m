...
for k=1:num_labels
    yk = (y ==k); % 1 or 0 if equal
    hk = htheta(:,k);
    J = J +(1/m)*( -yk' * log(hk) - (1 - yk') * log(1 - hk) )' ;
end    

theta1_unbias = Theta1(:,[2:end]); % do not include the bias
theta2_unbias = Theta2(:,[2:end]); 
regularize = (lambda/2/m) * ( sum(sum(theta1_unbias.^2)) + sum(sum(theta2_unbias.^2)) );
